    ...
    <article role="article" id="post_<?php the_ID()?>" <?php post_class()?>>
        <header>
            ...
            <p class="no-print text-muted" style="margin-bottom: 30px;">
              <?php if (has_category()) { ?>
                <i class="glyphicon glyphicon-folder-open"></i>&nbsp; <?php _e('Filed under', 'bst'); ?>: <?php the_category(', ') ?>&nbsp;
              <?php } ?>
              <?php if (has_tag()) { ?>
                <?php _e('With themes', 'bst'); ?>: <?php echo get_the_tag_list('',', ','') ?><br/>
              <?php } ?>
